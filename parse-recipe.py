...
#set Tbale
tableURLs = dynamodb.Table('URLs')
tableIngredient = dynamodb.Table('Ingredients')
tableRecipes = dynamodb.Table('Recipes')
testname = "AAAAAAAAAAAAAA"
numbers=0.347
response = tableIngredient.put_item(
	Item={
			'Name': testname,
			'recipes':{
					'Importance': "0.555",
...
response = tableURLs.scan(
	FilterExpression=Attr('url').begins_with('https')
	)
itemURL = response['Items']
...
for index_url in range(5,6):
	sleep(1)
	importance_number = 0
	getNumberofIngredient = 0
	geturl = itemURL[index_url]
	for key, value in geturl.items():
		url = value
	soup = None
	'''
	try:
		#url = "http://allrecipes.com/recipe/{}".format(recipeId)
		url = "https://allrecipes.com/recipe/98579/barbies-tuna-salad/"
	...
	if soup:
		...
		preparationSpan = soup.find("span", class_="ready-in-time")
		...
		#
		if preparationSpan != None:
				preparationTime = preparationSpan.text
				if "h" in preparationTime:
					print("Level of difficutly: Hard")
				else:
					print("Level of difficutly: Easy")
				print("Ready in: ", preparationTime)
		else:
				print("Undeine preparation Time and difficulty")
		...
		importance_number = 1/count
		print("importance number: ", importance_number)
		ingredients = []
		for i in range(0, count):
			...
			# delete "whole" at first index
			if parsedIngredient[0] == "whole":
				del parsedIngredient[0]
